...
class Missile(SphereCollideObj):
    ...
    def __init__(self, name, model_path, scale, position):
        ...
        SphereCollideObj.__init__(self, radius=10.0)
...
class Player(SpaceObject, SphereCollideObj):
    ...
    # -------------------------------------------------------
    # MOVEMENT SOUND — with fade‑in / fade‑out
    # -------------------------------------------------------
    def _play_movement_sound(self):
        """
        Start a looping engine sound with a smooth fade‑in.
        If already playing or fading in, do nothing.
        """
        sm = ShowBaseGlobal.base.sound

        # Already playing or fading in?
        ...
        if not snd:
            print("[Sound] Movement sound failed to start.")
            return

        snd.setLoop(True)
        snd.setVolume(0.0)  # start silent
        self._movement_sound = snd

        # Fade‑in task
        def fade_in(task):
            t = min(task.time / 0.35, 1.0)
            snd.setVolume(0.18 * t)  # target volume
            if t >= 1.0:
                self._movement_fade_task = None
                return task.done
            return task.cont

        self._movement_fade_task = ShowBaseGlobal.base.taskMgr.add(fade_in, "movementFadeIn")


    def _stop_movement_sound(self):
        """
        Fade out the engine sound smoothly, then stop it.
        """
...
class DroneDefender(SpaceObject, SphereCollideObj):
    def __init__(self, name, model_path, scale, position, orbit_radius=20, health=10):
        SpaceObject.__init__(self, name, model_path, scale, position, health)
        # Drone model is scaled to 10.0 in patterns → collider must be smaller
        SphereCollideObj.__init__(self, radius=6.0)
...
# ============================================================
class BoostRing(SphereCollideObj):
    def __init__(self, name, position, scale=20):
        ...
        # ----------------------------------------------------
        # Procedural ring model (two quads forming a circle)
        # ----------------------------------------------------
        cm = CardMaker("ring")
        cm.setFrame(-1, 1, -1, 1)

        # Outer ring
        outer = self.node.attachNewNode(cm.generate())
        outer.setScale(scale)
        outer.setBillboardPointEye()
        outer.setColorScale(1.2, 0.4, 0.1, 1)
        outer.setTransparency(TransparencyAttrib.MAlpha)

        # Inner cutout
        inner = self.node.attachNewNode(cm.generate())
        inner.setScale(scale * 0.6)
        inner.setBillboardPointEye()
        inner.setColorScale(0, 0, 0, 1)
        inner.setTransparency(TransparencyAttrib.MAlpha)

        # Spin animation
        ShowBaseGlobal.base.taskMgr.add(self._spin, f"spin_{name}")

        # Collider (sphere)
        SphereCollideObj.__init__(self, radius=scale * 0.25, debug=True)
        self.debug_mode = True

    def _spin(self, task):
        self.node.setH(self.node.getH() + 60 * task.dt)
        return task.cont
